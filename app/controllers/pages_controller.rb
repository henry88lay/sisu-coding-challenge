class PagesController < ApplicationController
  ...
  def home
    ...
    action_total = @movies.inject(0) do |sum,movie| 
      if movie_is_genre?(movie, genre)
        sum += movie.vote_average 
      end
      sum
    end
    action_movies_count = @movies.select{|movie| movie_is_genre?(movie, genre) }.size
    @action_avg = (action_total / action_movies_count).round(2)
  end
